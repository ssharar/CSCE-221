...
public:
  ...
  DListNode<T>* insert_after(int k, T d); // insert the int after this node
  // return a pointer to the inserted node
  ...
  int getListSize() { return representative->listSize; }
...
template <typename T>
DListNode<T>* DListNode<T>::insert_after(int k, T d) {
  DListNode<T>* temp = new DListNode(k, d);   // 2 OPERATION
